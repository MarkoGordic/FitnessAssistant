...
public class PermissionFunctional {
    ...
    public static void checkFineLocationPermission(Context context, ActivityResultLauncher<String> permissionLauncher){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        ...
        } else if (ContextCompat.checkSelfPermission(context, "com.google.android.gms.permission.ACCESS_FINE_LOCATION") != PackageManager.PERMISSION_GRANTED) {
            // alert dialog to let user know we're requesting activity recognition
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setView(R.layout.custom_ok_alert_dialog);
            AlertDialog dialog = builder.create();
            ...
            dialog.findViewById(R.id.dialog_ok_button).setOnClickListener(view2 -> {
                dialog.dismiss();
                permissionLauncher.launch("com.google.android.gms.permission.ACCESS_FINE_LOCATION");
            });
        } else {
            // Start needed services
        }
    ...
    public static void checkBackgroundLocationPermission(Context context, ActivityResultLauncher<String> permissionLauncher){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        ...
        } else if (ContextCompat.checkSelfPermission(context, "com.google.android.gms.permission.ACCESS_BACKGROUND_LOCATION") != PackageManager.PERMISSION_GRANTED) {
            // alert dialog to let user know we're requesting activity recognition
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setView(R.layout.custom_ok_alert_dialog);
            AlertDialog dialog = builder.create();
            ...
            dialog.findViewById(R.id.dialog_ok_button).setOnClickListener(view2 -> {
                dialog.dismiss();
                permissionLauncher.launch("com.google.android.gms.permission.ACCESS_BACKGROUND_LOCATION");
            });
        } else {
            // Start needed services
        }
